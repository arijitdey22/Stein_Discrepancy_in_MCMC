...
thin.comp <- function(rep = 1e2, n.all = c(50, 100, 200, 500, 1000)){
  ...
  for (i.n in 1:length(n.all))
  {
    for (i.rep in 1:rep)
    {
      ...
      samp <- as.matrix(draw.mvn.mcmc(n.cur, d = d, mu = mu.true, sigma = sigma.true)[[1]])
...
save(out, file = "04_Thin_comp.Rdata")

#----------------------
#rm(list = ls())

load("04_Thin_comp.Rdata")
